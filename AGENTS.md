...
Core code lives in `src/agentic_rag/`. Keep CLI entry points in `cli.py` and `__main__.py`, shared orchestration in `service.py`, compatibility wrappers in `app.py`, graph logic in `graph.py`, provider integrations in `providers.py`, typed failures in `errors.py`, document loading in `documents.py`, and configuration in `settings.py`. Tests live in `tests/` and follow the module split, for example `tests/test_graph.py`, `tests/test_service.py`, and `tests/test_providers.py`. Runtime configuration starts from `.env.example`; local Qdrant index data is written under `.cache/vectorstores/`.
...
## Testing Guidelines
Tests use `pytest` with `src` on the Python path. Add or update tests whenever graph behavior, provider resolution, or CLI-facing settings change. Prefer deterministic unit tests with fake models or tools over live provider calls. Name tests `test_<behavior>()` and group them by module under `tests/`.
